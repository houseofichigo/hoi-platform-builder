...
describe("deriveGovernanceFlags · EU AI Act", () => {
  it("EU_AI_ACT_HIGH_RISK is hard_stop for high-risk function + material impact", () => {
  ...
  it("EU_AI_ACT_HIGH_RISK is requires_action for partial match", () => {
    const f = deriveGovernanceFlags({
      useCaseFunction: "finance",
      capture: {},
    });
  ...
  it("CONFORMITY_ASSESSMENT only fires when high-risk AI is a hard stop", () => {
  ...
  it("TRANSPARENCY_ART13 fires from customer-facing signals", () => {
    expect(codesOf(deriveGovernanceFlags({ capture: { customer_facing_choice: "yes" } }))).toContain(
      "TRANSPARENCY_ART13",
    );
    expect(codesOf(deriveGovernanceFlags({ capture: { target_domain: "customer_facing" } }))).toContain(
      "TRANSPARENCY_ART13",
    );
    expect(codesOf(deriveGovernanceFlags({ capture: { target_domain: "back_office" } }))).not.toContain(
      "TRANSPARENCY_ART13",
    );
  });

  it("ARTICLE_11_DOCUMENTATION fires only when stage=production", () => {
    expect(codesOf(deriveGovernanceFlags({ capture: {}, stage: "production" }))).toContain(
      "ARTICLE_11_DOCUMENTATION",
    );
    expect(codesOf(deriveGovernanceFlags({ capture: {}, stage: "pilot" }))).not.toContain(
      "ARTICLE_11_DOCUMENTATION",
    );
  });
});
...
describe("deriveGovernanceFlags · GDPR", () => {
  ...
  it("RIGHT_TO_EXPLANATION fires from explicit automated-decision signal", () => {
    const f = deriveGovernanceFlags({
  ...
  it("RIGHT_TO_EXPLANATION is inferred from personal data + model-based decisions", () => {
    const f = deriveGovernanceFlags({
      capture: {
        personal_data_choice: "yes",
        decision_logic_type: "model_based",
...
describe("deriveGovernanceFlags · Internal policy", () => {
  ...
  it("CHANGE_MANAGEMENT only fires on Pilot -> Production", () => {
    expect(
      codesOf(
...
describe("deriveGovernanceFlags · idempotence + integration", () => {
  ...
  it("a maximal trigger set produces the expected EU/HOI rule codes", () => {
    const f = deriveGovernanceFlags({
      useCaseFunction: "finance",
      capture: {
        personal_data_choice: "yes",
