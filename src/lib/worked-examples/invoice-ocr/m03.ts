...
export const M03_OCR_CONTENT = {
  ...
  promptAssignments: [
    ...
    },
    {
      ...
      why:
        "The assistant also needs context: VAT rules, EU AI Act classification, GDPR posture, internal approval policy. This prompt produces a research report the assistant can read.",
      scaffoldKey: "context",
    },
    {
      id: "mock",
...
export function getM03PromptScaffolds(
  ...
  return {
    ...
    context: {
      ...
      role: `You are a compliance research analyst with expertise in EU VAT, GDPR, the EU AI Act, and ${country}-specific accounting regulation.`,
      context: `The report becomes a knowledge base source for the M04 OCR assistant. The assistant uses it to apply the right rule at the right moment — VAT rates for line items, GDPR posture for supplier contacts, AI Act classification for the workflow itself, internal approval thresholds. The audience is the assistant, not a human reader.`,
      ...
      output: `Return a structured report with these sections:\n· VAT regime (standard + reduced rates that apply to AP)\n· GDPR posture (lawful basis for supplier contacts, RoPA reference)\n· EU AI Act classification (low/limited/high, with Annex III analysis)\n· Internal approval policy template (thresholds + signatures)\n· Cross-border rules (SCC coverage, residency requirements)\n· Sources (URL + access date per claim)`,
