...
-- Idempotent re-application that mirrors supabase/migrations/20260627090000_build_process_workspace_foundation.sql
-- and adds the missing Data API grants on the four new Build tables.
...
alter table public.workspace_invitations
  add column if not exists department_id uuid references public.department(id) on delete set null,
  add column if not exists manager_member_id uuid references public.workspace_members(id) on delete set null;
create index if not exists workspace_members_department_idx on public.workspace_members(department_id) where department_id is not null;
create index if not exists workspace_members_manager_idx on public.workspace_members(manager_member_id) where manager_member_id is not null;
create index if not exists workspace_invitations_department_idx on public.workspace_invitations(department_id) where department_id is not null;
create index if not exists workspace_invitations_manager_idx on public.workspace_invitations(manager_member_id) where manager_member_id is not null;
...
declare v_parent uuid; v_depth integer := 0;
begin
  if new.parent_id is null then return new; end if;
  if new.parent_id = new.id then raise exception 'department.parent_id cannot reference itself' using errcode = '23514'; end if;
  select parent_id into v_parent from public.department where id = new.parent_id and workspace_id = new.workspace_id;
  while v_parent is not null loop
    v_depth := v_depth + 1;
    if v_depth > 100 then raise exception 'department parent chain exceeds maximum depth' using errcode = '23514'; end if;
    if v_parent = new.id then raise exception 'department.parent_id would create a cycle' using errcode = '23514'; end if;
    select parent_id into v_parent from public.department where id = v_parent and workspace_id = new.workspace_id;
  end loop;
  return new;
end; $$;
drop trigger if exists department_parent_cycle_guard on public.department;
create trigger department_parent_cycle_guard before insert or update of parent_id on public.department
...
returns trigger language plpgsql set search_path = public as $$
declare v_manager uuid; v_depth integer := 0;
begin
  if new.manager_member_id is null then return new; end if;
  if new.manager_member_id = new.id then raise exception 'workspace_members.manager_member_id cannot reference itself' using errcode = '23514'; end if;
  select manager_member_id into v_manager from public.workspace_members where id = new.manager_member_id and workspace_id = new.workspace_id;
  while v_manager is not null loop
    v_depth := v_depth + 1;
    if v_depth > 100 then raise exception 'workspace member manager chain exceeds maximum depth' using errcode = '23514'; end if;
    if v_manager = new.id then raise exception 'workspace_members.manager_member_id would create a cycle' using errcode = '23514'; end if;
    select manager_member_id into v_manager from public.workspace_members where id = v_manager and workspace_id = new.workspace_id;
  end loop;
  return new;
end; $$;
drop trigger if exists workspace_member_manager_cycle_guard on public.workspace_members;
create trigger workspace_member_manager_cycle_guard before insert or update of manager_member_id on public.workspace_members
...
begin
  if new.parent_id is not null then
    ...
    if v_workspace_id is distinct from new.workspace_id then raise exception 'department parent must belong to the same workspace' using errcode = '23514'; end if;
  end if;
  if new.lead_member_id is not null then
    ...
    if v_workspace_id is distinct from new.workspace_id then raise exception 'department lead must belong to the same workspace' using errcode = '23514'; end if;
  end if;
  return new;
end; $$;
drop trigger if exists department_workspace_guard on public.department;
create trigger department_workspace_guard before insert or update of parent_id, lead_member_id, workspace_id on public.department
...
begin
  if new.department_id is not null then
    ...
    if v_workspace_id is distinct from new.workspace_id then raise exception 'member department must belong to the same workspace' using errcode = '23514'; end if;
  end if;
  if new.manager_member_id is not null then
    ...
    if v_workspace_id is distinct from new.workspace_id then raise exception 'manager must belong to the same workspace' using errcode = '23514'; end if;
  end if;
  return new;
end; $$;
drop trigger if exists workspace_member_structure_guard on public.workspace_members;
create trigger workspace_member_structure_guard before insert or update of department_id, manager_member_id, workspace_id on public.workspace_members
...
begin
  if new.department_id is not null then
    select workspace_id into v_workspace_id from public.department where id = new.department_id;
    if v_workspace_id is distinct from new.workspace_id then raise exception 'process department must belong to the same workspace' using errcode = '23514'; end if;
  end if;
  if new.owner_member_id is not null then
    ...
    if v_workspace_id is distinct from new.workspace_id then raise exception 'process owner must belong to the same workspace' using errcode = '23514'; end if;
  end if;
  return new;
end; $$;
drop trigger if exists process_workspace_guard on public.process;
create trigger process_workspace_guard before insert or update of workspace_id, department_id, owner_member_id on public.process
...
returns trigger language plpgsql set search_path = public as $$
declare v_process_workspace_id uuid; v_workspace_id uuid;
begin
  select workspace_id into v_process_workspace_id from public.process where id = new.process_id;
  if v_process_workspace_id is null then raise exception 'process step must reference an existing process' using errcode = '23503'; end if;
  if new.workspace_id is null then new.workspace_id := v_process_workspace_id; end if;
  if new.workspace_id is distinct from v_process_workspace_id then raise exception 'process step workspace must match process workspace' using errcode = '23514'; end if;
  if new.actor_member_id is not null then
    ...
    if v_workspace_id is distinct from new.workspace_id then raise exception 'step actor must belong to the same workspace' using errcode = '23514'; end if;
  end if;
  if new.department_id is not null then
    select workspace_id into v_workspace_id from public.department where id = new.department_id;
    if v_workspace_id is distinct from new.workspace_id then raise exception 'step department must belong to the same workspace' using errcode = '23514'; end if;
  end if;
  return new;
end; $$;
drop trigger if exists process_step_workspace_guard on public.process_step;
create trigger process_step_workspace_guard before insert or update of workspace_id, process_id, actor_member_id, department_id on public.process_step
...
begin
  if to_regclass('public.department') is not null then
    ...
    create trigger department_set_updated_at before update on public.department for each row execute function public.set_updated_at();
  end if;
  if to_regclass('public.process') is not null then
    drop trigger if exists process_set_updated_at on public.process;
    create trigger process_set_updated_at before update on public.process for each row execute function public.set_updated_at();
  end if;
  if to_regclass('public.process_step') is not null then
    ...
    create trigger process_step_set_updated_at before update on public.process_step for each row execute function public.set_updated_at();
  end if;
end $$;

-- ===== Data API grants (missing from original migration) =====
grant select, insert, update, delete on public.department to authenticated;
grant all on public.department to service_role;
grant select, insert, update, delete on public.process to authenticated;
grant all on public.process to service_role;
grant select, insert, update, delete on public.process_step to authenticated;
grant all on public.process_step to service_role;
grant select, insert on public.process_status_audit to authenticated;
grant all on public.process_status_audit to service_role;
...
create policy department_select_workspace_members on public.department for select to authenticated
using (public.is_workspace_member(workspace_id, auth.uid()));

drop policy if exists department_admin_write on public.department;
create policy department_admin_write on public.department for all to authenticated
...
create policy process_select_workspace_members on public.process for select to authenticated
using (public.is_workspace_member(workspace_id, auth.uid()));

drop policy if exists process_insert_member_plus on public.process;
create policy process_insert_member_plus on public.process for insert to authenticated
with check (
...
create policy process_update_allowed_actors on public.process for update to authenticated
using (
  ...
  or exists (
    ...
    where d.id = process.department_id and d.workspace_id = process.workspace_id and wm.user_id = auth.uid()
  )
)
with check (
  public.has_workspace_role(workspace_id, auth.uid(), array['admin','owner'])
  or (created_by = auth.uid() and status in ('draft','changes_requested','submitted'))
  or exists (
    select 1 from public.department d
    join public.workspace_members wm on wm.id = d.lead_member_id
    where d.id = process.department_id and d.workspace_id = process.workspace_id and wm.user_id = auth.uid()
...
create policy process_step_select_workspace_members on public.process_step for select to authenticated
using (public.is_workspace_member(workspace_id, auth.uid()));

drop policy if exists process_step_write_allowed_actors on public.process_step;
create policy process_step_write_allowed_actors on public.process_step for all to authenticated
using (
  public.has_workspace_role(workspace_id, auth.uid(), array['admin','owner'])
  or exists (
    select 1 from public.process p
    where p.id = process_step.process_id and p.workspace_id = process_step.workspace_id
      and p.created_by = auth.uid() and p.status in ('draft','changes_requested')
  )
)
with check (
  public.has_workspace_role(workspace_id, auth.uid(), array['admin','owner'])
...
create policy process_status_audit_select_workspace_members on public.process_status_audit for select to authenticated
using (public.is_workspace_member(workspace_id, auth.uid()));

drop policy if exists process_status_audit_admin_insert on public.process_status_audit;
create policy process_status_audit_admin_insert on public.process_status_audit for insert to authenticated
with check (public.has_workspace_role(workspace_id, auth.uid(), array['admin','owner']));

-- ===== RPCs =====
create or replace function public.decide_process(p_process_id uuid, p_decision text, p_note text default null)
returns public.process language plpgsql security definer set search_path = public as $$
declare v_process public.process; v_from public.process_status; v_to public.process_status;
begin
  if auth.uid() is null then raise exception 'not authenticated' using errcode = '42501'; end if;
  select * into v_process from public.process where id = p_process_id for update;
  if not found then raise exception 'process not found' using errcode = 'P0002'; end if;
  ...
  if v_to is null then raise exception 'invalid process decision: %', p_decision using errcode = '22023'; end if;
  update public.process set
    status = v_to,
    ...
    updated_at = now()
  where id = p_process_id returning * into v_process;
  insert into public.process_status_audit (workspace_id, process_id, from_status, to_status, changed_by, note)
  values (v_process.workspace_id, v_process.id, v_from, v_to, auth.uid(), p_note);
  return v_process;
end; $$;
...
begin
  if auth.uid() is null then raise exception 'not authenticated' using errcode = '42501'; end if;
  if not public.is_workspace_member(p_workspace_id, auth.uid()) then raise exception 'not a workspace member' using errcode = '42501'; end if;
  select jsonb_build_object(
    ...
    'departments', (select count(*) from public.department d where d.workspace_id = p_workspace_id and d.archived_at is null)
  ) into v_counts
  from public.process p where p.workspace_id = p_workspace_id;
  return coalesce(v_counts, jsonb_build_object(
    'workspace_id', p_workspace_id,'total',0,'draft',0,'submitted',0,'under_review',0,
    'changes_requested',0,'approved',0,'awaiting_decision',0,'departments',0
  ));
end; $$;
